...
const Chat = () => {
    ...
    const getUserInfoList = async () => {
        ...
        if (userInfoList.length === 0 && window.location.hostname !== "127.0.0.1") {
            ...
            const nameInitials = getInitials(userInfoList[0].user_id);
            setUserId(nameInitials);
            setShowUserId(true);
            setShowAuthMessage(false);
        }
    }
    ...
    const onShowCitation = (citation: Citation) => {
        ...
        if (citation.url) {
            window.open(decodeURIComponent(citation.url), "_blank");
        }
    };
                                    ...
                                    <>
                                        {answer.role === "user" ? (
                                            <div className={styles.chatMessageUser} tabIndex={0}>
                                                ...
                                                { showUserId ?   <div className={styles.chatMessageUserMessageUser}>{userId}</div> : null}
                                            </div>
                                        ) : (
                                            answer.role === "assistant" ? <div className={styles.chatMessageGpt}>
                                                <div className={styles.botIcon}>
                                                <img src={SiemensUser} alt="Siemens" />
                                                </div>
                                                <Answer
                                                    answer={{
